...
filterGameDataByAccountName(durak::GameData const &gameData, std::string const &accountName) {
    ...
    for (auto &player: filteredGameData.players | ranges::views::filter(
            [&accountName](auto const &player) { return player.name != accountName; })) {
        ranges::transform(player.cards, player.cards.begin(),
                          [](boost::optional<durak::Card> const &) { return boost::optional<durak::Card>{}; });
    }
    return filteredGameData;
}

auto const &moveMapping = std::map<durak::Move, shared_class::Move>{{durak::Move::startAttack, shared_class::Move::AddCards},
                                                                    {durak::Move::addCard,     shared_class::Move::AddCards},
                                                                    {durak::Move::pass,        shared_class::Move::AttackAssistPass},
                                                                    {durak::Move::defend,      shared_class::Move::Defend},
                                                                    {durak::Move::takeCards,   shared_class::Move::TakeCards}};
...
std::vector<shared_class::Move>
calculateAllowedMovesWithPassState(durak::Game const &game, durak::PlayerRole playerRole,
                                   PassAttackAndAssist passAttackAndAssist) {
    auto allowedMoves = calculateAllowedMoves(game, playerRole);
    if (auto takeCards = ranges::find(allowedMoves, shared_class::Move::TakeCards); takeCards != allowedMoves.end() and
                                                                                    ranges::find(allowedMoves,
                                                                                                 shared_class::Move::Defend) ==
                                                                                    allowedMoves.end() and playerRole ==
                                                                                                           durak::PlayerRole::defend and
                                                                                    passAttackAndAssist.attack and
                                                                                    passAttackAndAssist.assist) {
...
calcAllowedMoves(durak::Game const &game, durak::PlayerRole playerRole,
                 std::optional<std::vector<shared_class::Move> > const &removeFromAllowedMoves,
                 std::optional<std::vector<shared_class::Move> > const &addToAllowedMoves) {
    auto allowedMoves = shared_class::DurakAllowedMoves{
            removeFromAllowedMoves.value_or(calculateAllowedMoves(game, playerRole))};
    if (addToAllowedMoves && not addToAllowedMoves->empty()) {
        allowedMoves.allowedMoves.insert(allowedMoves.allowedMoves.end(), addToAllowedMoves.value().begin(),
                                         addToAllowedMoves.value().end());
...
void
sendAvailableMoves(durak::Game const &game, std::list<User> const &users,
                   AllowedMoves const &removeFromAllowedMoves = {}, AllowedMoves const &addToAllowedMoves = {}) {
    if (auto attackingPlayer = game.getAttackingPlayer()) {
        if (auto attackingUser = ranges::find_if(users, [attackingPlayerName = attackingPlayer->id](User const &user) {
                return user.accountName == attackingPlayerName;
            }); attackingUser != users.end()) {
            attackingUser->sendMsgToUser(objectToStringWithObjectName(
                    calcAllowedMoves(game, durak::PlayerRole::attack, removeFromAllowedMoves.attack,
                                     addToAllowedMoves.attack)));
        }
    }
    if (auto assistingPlayer = game.getAssistingPlayer()) {
        if (auto assistingUser = ranges::find_if(users, [assistingPlayerName = assistingPlayer->id](User const &user) {
                return user.accountName == assistingPlayerName;
            }); assistingUser != users.end()) {
            assistingUser->sendMsgToUser(objectToStringWithObjectName(
                    calcAllowedMoves(game, durak::PlayerRole::assistAttacker, removeFromAllowedMoves.assist,
                                     addToAllowedMoves.assist)));
        }
    }
    if (auto defendingPlayer = game.getDefendingPlayer()) {
        if (auto defendingUser = ranges::find_if(users, [defendingPlayerName = defendingPlayer->id](User const &user) {
                return user.accountName == defendingPlayerName;
            }); defendingUser != users.end()) {
            defendingUser->sendMsgToUser(objectToStringWithObjectName(
                    calcAllowedMoves(game, durak::PlayerRole::defend, removeFromAllowedMoves.defend,
                                     addToAllowedMoves.defend)));
        }
    }
}
...
sendGameDataToAccountsInGame(durak::Game const &game, std::list<User> const &users,
    ...
    ranges::for_each(gameData.players, [](auto &player) {
        ranges::sort(player.cards, [](auto const &card1, auto const &card2) { return card1.value() < card2.value(); });
    });
    ranges::for_each(users, [&gameData, opponentCards](User const &user) {
        user.sendMsgToUser(objectToStringWithObjectName(
                opponentCards == shared_class::OpponentCards::showNumberOfOpponentCards ? filterGameDataByAccountName(
                        gameData, user.accountName) : gameData));
    });
}
...
struct GameDependencies {
    GameDependencies(matchmaking_game::StartGame const &startGame, std::string const &gameName_,
                     std::list<User> &&users_, boost::asio::io_context &ioContext_,
                     boost::asio::ip::tcp::endpoint const &gameToMatchmakingEndpoint_) : users{std::move(users_)},
                                                                                         isRanked{startGame.ratedGame},
                                                                                         gameName{gameName_},
                                                                                         ioContext{ioContext_},
                                                                                         gameToMatchmakingEndpoint{
...
auto const timerActive = [](GameDependencies &gameDependencies) {
    return gameDependencies.timerOption.timerType != shared_class::TimerType::noTimer;
};
...
sendGameOverToMatchmaking(matchmaking_game::GameOver gameOver, GameDependencies &gameDependencies) {
    ...
    co_await ws->async_handshake(gameDependencies.gameToMatchmakingEndpoint.address().to_string() +
                                 std::to_string(gameDependencies.gameToMatchmakingEndpoint.port()), "/");
    static size_t id = 0;
    auto myWebsocket = MyWebsocket<Websocket>{std::move(ws), "sendGameOverToMatchmaking",
                                              fmt::fg(fmt::color::cornflower_blue), std::to_string(id++)};
    ...
    if (splitMessage.size() == 2) {
        ...
        if (typeToSearch == "GameOverSuccess") {
            ...
            std::cout << "GameOverError: " << stringToObject<matchmaking_game::GameOverError>(objectAsString).error
                      << std::endl;
        } else {
            std::cout << "matchmaking answered with: '" << msg << "' expected GameOverSuccess|{} or GameOverError|{} "
                      << std::endl;
        }
    } else {
        std::cout << "Game server answered with: '" << msg << "' expected GameOverSuccess|{} or GameOverError|{} "
                  << std::endl;
    }
    ranges::for_each(gameDependencies.users, [](User &user) {
        user.sendMsgToUser(objectToStringWithObjectName(matchmaking_game::LeaveGameSuccess{}));
    });
}
...
auto const handleGameOver = [](boost::optional<durak::Player> const &durak, GameDependencies &gameDependencies) {
    ...
    auto gameOver = matchmaking_game::GameOver{gameDependencies.gameName, gameDependencies.isRanked, std::move(winners),
                                               std::move(losers), std::move(draws)};
    co_spawn(gameDependencies.ioContext, sendGameOverToMatchmaking(std::move(gameOver), gameDependencies),
             printException);
};
...
auto const sendTimer = [](GameDependencies &gameDependencies) {
    if (gameDependencies.timerOption.timerType != shared_class::TimerType::noTimer) {
        ...
        ranges::for_each(gameDependencies.users, [&durakTimers](User const &user) {
            if (user.pausedTime) {
                durakTimers.pausedTimeUserDurationMilliseconds.push_back(
                        std::make_pair(user.accountName, user.pausedTime->count()));
            } else {
                using namespace std::chrono;
                durakTimers.runningTimeUserTimePointMilliseconds.push_back(std::make_pair(user.accountName,
                                                                                          duration_cast<milliseconds>(
                                                                                                  user.timer->expiry().time_since_epoch()).count()));
            }
        });
        ranges::for_each(gameDependencies.users, [&durakTimers](User const &user) {
            user.sendMsgToUser(objectToStringWithObjectName(durakTimers));
        });
    }
};

auto const initTimerHandler = [](GameDependencies &gameDependencies,
                                 boost::sml::back::process<resumeTimer> process_event) {
    using namespace std::chrono;
    ranges::for_each(gameDependencies.users,
                     [&gameDependencies](auto &user) { user.pausedTime = gameDependencies.timerOption.timeAtStart; });
...
auto const nextRoundTimerHandler = [](GameDependencies &gameDependencies,
                                      boost::sml::back::process<resumeTimer, sendTimerEv> process_event) {
    using namespace std::chrono;
    ranges::for_each(gameDependencies.users, [&timerOption = gameDependencies.timerOption](auto &user) {
        if (timerOption.timerType == shared_class::TimerType::addTimeOnNewRound) {
            user.pausedTime = timerOption.timeForEachRound +
                              duration_cast<milliseconds>(user.timer->expiry() - system_clock::now());
        } else {
            user.pausedTime = timerOption.timeForEachRound;
        }
        user.timer->cancel();
    ...
    if (auto user = ranges::find_if(gameDependencies.users, [userName = userReloggedEv.accountName](auto const &_user) {
        ...
        user->sendMsgToUser(objectToStringWithObjectName(shared_class::DurakAllowedMoves{
                calculateAllowedMoves(gameDependencies.game,
                                      gameDependencies.game.getRoleForName(userReloggedEv.accountName))}));
    }
};
        ...
        if (auto user = ranges::find_if(gameDependencies.users,
            ...
            user->sendMsgToUser(objectToStringWithObjectName(shared_class::DurakAllowedMoves{
                    {shared_class::Move::AnswerDefendWantsToTakeCardsYes,
                     shared_class::Move::AnswerDefendWantsToTakeCardsNo}}));
        }
    }
};

auto const defendsWantsToTakeCardsSendMovesToAttackOrAssist = [](durak::Game &game, durak::PlayerRole playerRole,
                                                                 User &user) {
    auto allowedMoves = calculateAllowedMoves(game, playerRole);
    if (ranges::find_if(allowedMoves, [](auto allowedMove) { return allowedMove == shared_class::Move::AddCards; }) !=
        allowedMoves.end()) {
        user.sendMsgToUser(objectToStringWithObjectName(shared_class::DurakAllowedMoves{
                {shared_class::Move::AttackAssistDoneAddingCards, shared_class::Move::AddCards}}));
    } else {
        user.sendMsgToUser(objectToStringWithObjectName(
                shared_class::DurakAllowedMoves{{shared_class::Move::AttackAssistDoneAddingCards}}));
    }
};
...
auto const userReloggedInAskAttackAssist = [](GameDependencies &gameDependencies, userRelogged const &userReloggedEv) {
    if (auto user = ranges::find_if(gameDependencies.users, [userName = userReloggedEv.accountName](auto const &_user) {
        ...
        if ((not gameDependencies.passAttackAndAssist.assist &&
            ...
            (not gameDependencies.passAttackAndAssist.attack &&
             gameDependencies.game.getRoleForName(userReloggedEv.accountName) == durak::PlayerRole::attack)) {
            defendsWantsToTakeCardsSendMovesToAttackOrAssist(gameDependencies.game,
                                                             gameDependencies.game.getRoleForName(
                                                                     userReloggedEv.accountName), *user);
        }
    }
};
auto const blockEverythingExceptStartAttack = AllowedMoves{.defend = std::vector<shared_class::Move>{}, .attack = std::vector<shared_class::Move>{
        shared_class::Move::AddCards}, .assist = std::vector<shared_class::Move>{}};
...
auto const resumeTimerHandler = [](GameDependencies &gameDependencies, resumeTimer const &resumeTimerEv) {
    ranges::for_each(gameDependencies.users,
                     [playersToResume = resumeTimerEv.playersToResume, &gameDependencies](User &user) {
                         if (ranges::find(playersToResume, user.accountName) != playersToResume.end()) {
                             ...
                             user.pausedTime = {};
                             co_spawn(
                                     user.timer->get_executor(), [playersToResume = std::move(
                                             playersToResume), &gameDependencies, &user]() {
                                         return runTimer(user.timer, user.accountName, gameDependencies);
                                     }, printException);
                         }
                     });
};
...
auto const isAttackingOrAssistingPlayer = [](GameDependencies &gameDependencies,
                                             std::tuple<shared_class::DurakAttack, User &> const &attackEvAndUser) {
...
auto const setAttackAnswer = [](GameDependencies &gameDependencies,
                                std::tuple<shared_class::DurakAttackPass, User &> const &attackPassEventAndUser,
                                boost::sml::back::process<pauseTimer, sendTimerEv> process_event) {
    ...
    if (not gameDependencies.passAttackAndAssist.attack) {
        if (gameDependencies.game.getRoleForName(user.accountName) == durak::PlayerRole::attack) {
            ...
            user.sendMsgToUser(objectToStringWithObjectName(shared_class::DurakAllowedMoves{{}}));
            user.sendMsgToUser(objectToStringWithObjectName(
                    shared_class::DurakDefendWantsToTakeCardsFromTableDoneAddingCardsSuccess{}));
        } else {
            user.sendMsgToUser(objectToStringWithObjectName(
                    shared_class::DurakDefendWantsToTakeCardsFromTableDoneAddingCardsError{"role is not attack"}));
        }
    } else {
        user.sendMsgToUser(objectToStringWithObjectName(
                shared_class::DurakDefendWantsToTakeCardsFromTableDoneAddingCardsError{"pass already set"}));
    }
};

auto const setAssistAnswer = [](GameDependencies &gameDependencies,
                                std::tuple<shared_class::DurakAssistPass, User &> const &assistPassEventAndUser,
                                boost::sml::back::process<pauseTimer, sendTimerEv> process_event) {
    ...
    if (not gameDependencies.passAttackAndAssist.assist) {
        if (gameDependencies.game.getRoleForName(user.accountName) == durak::PlayerRole::assistAttacker) {
            ...
            user.sendMsgToUser(objectToStringWithObjectName(
                    shared_class::DurakDefendWantsToTakeCardsFromTableDoneAddingCardsError{"role is not assist"}));
        }
    } else {
        user.sendMsgToUser(objectToStringWithObjectName(
                shared_class::DurakDefendWantsToTakeCardsFromTableDoneAddingCardsError{"pass already set"}));
...
auto const checkData = [](GameDependencies &gameDependencies, boost::sml::back::process<askDef> process_event) {
    if (auto attackingPlayer = gameDependencies.game.getAttackingPlayer(); not attackingPlayer ||
                                                                           attackingPlayer->getCards().empty()) {
        gameDependencies.passAttackAndAssist.attack = true;
    }
    if (auto assistingPlayer = gameDependencies.game.getAssistingPlayer(); not assistingPlayer ||
                                                                           assistingPlayer->getCards().empty()) {
        gameDependencies.passAttackAndAssist.assist = true;
    }
    if (gameDependencies.passAttackAndAssist.assist && gameDependencies.passAttackAndAssist.attack &&
        gameDependencies.game.countOfNotBeatenCardsOnTable() == 0) {
        process_event(askDef{});
    }
};

auto const checkAttackAndAssistAnswer = [](GameDependencies &gameDependencies,
                                           boost::sml::back::process<chill, nextRoundTimer, sendTimerEv> process_event) {
    if (auto attackingPlayer = gameDependencies.game.getAttackingPlayer(); not attackingPlayer ||
                                                                           attackingPlayer->getCards().empty()) {
        gameDependencies.passAttackAndAssist.attack = true;
    }
...
auto const startAskDef = [](GameDependencies &gameDependencies,
...
auto const userLeftGame = [](GameDependencies &gameDependencies,
                             std::tuple<shared_class::DurakLeaveGame, User &> const &leaveGameEventUser) {
...
hasToMove(durak::Game const &game, durak::PlayerRole playerRole, PassAttackAndAssist passAttackAndAssist,
    ...
    switch (playerRole) {
        case PlayerRole::attack: {
            if (passAttackAndAssist.attack == false and
                (game.getTableAsVector().size() % 2 == 0 or std::same_as<CurrentStateType, AskAttackAndAssist>)) {
                return true;
            } else {
                return false;
            }
        ...
        case PlayerRole::assistAttacker: {
            ...
            if (game.getAttackStarted() and passAttackAndAssist.assist == false and
                (game.getTableAsVector().size() % 2 == 0 or std::same_as<CurrentStateType, AskAttackAndAssist>)) {
                return true;
            } else {
                return false;
        ...
        case PlayerRole::defend: {
            if (hasToMove(game, durak::PlayerRole::attack, passAttackAndAssist, currentState) or
                hasToMove(game, durak::PlayerRole::assistAttacker, passAttackAndAssist, currentState)) {
                return false;
            } else {
                return true;
            }
...
calcNextMove(std::optional<durak_computer_controlled_opponent::Action> const &action,
             std::vector<shared_class::Move> const &moves, durak::PlayerRole const &playerRole,
             std::vector<std::tuple<uint8_t, durak::Card> > const &defendIdCardMapping,
             std::vector<std::tuple<uint8_t, durak::Card> > const &attackIdCardMapping, auto const &currentState) {
    using CurrentStateType = std::decay_t<decltype(currentState)>;
    if (playerRole == durak::PlayerRole::attack) {
        ...
        if (action and action.value().value() == 253) {
            ...
            if (auto idCard = ranges::find_if(defendIdCardMapping, [value = action->value()](auto const &idAndCard) {
                    return value == std::get<0>(idAndCard);
                }); idCard != defendIdCardMapping.end()) {
...
nextMove(GameDependencies &gameDependencies, std::tuple<shared_class::DurakNextMove, User &> const &durakNextMoveUser,
    ...
    if (durak_computer_controlled_opponent::tableValidForMoveLookUp(gameDependencies.game.getTable())) {
        ...
        if (hasToMove(gameDependencies.game, playerRole, gameDependencies.passAttackAndAssist, currentState)) {
            ...
            auto const [compressedCardsForAttack, compressedCardsForDefend, compressedCardsForAssist] = calcIdAndCompressedCardsForAttackAndDefend(
            ...
            auto const &someRound = confu_soci::findStruct<database::Round>(sql, "gameState",
                                                                            database::gameStateAsString(
                                                                                    {attackCardsCompressed,
                                                                                     defendCardsCompressed},
                                                                                    gameDependencies.game.getTrump()));
            if (someRound) {
                if (someRound->combination.empty()) {
                    ...
                    user.sendMsgToUser(objectToStringWithObjectName(
                            shared_class::DurakNextMoveError{"Unsupported card combination."}));
                } else {
                    ...
                    auto const &calculatedNextMove = calcNextMove(actionForRole, allowedMoves, playerRole,
                                                                  compressedCardsForDefend, compressedCardsForAttack,
                                                                  currentState);
                    if (calculatedNextMove) {
                        user.sendMsgToUser(objectToStringWithObjectName(*calculatedNextMove));
                    } else {
                        user.sendMsgToUser(objectToStringWithObjectName(shared_class::DurakNextMoveError{
                                "could not find a move. Are you sure you have to Move?"}));
                    }
                }
                ...
                user.sendMsgToUser(objectToStringWithObjectName(
                        shared_class::DurakNextMoveError{"could not find a game for the cards."}));
            }
        } else {
            user.sendMsgToUser(objectToStringWithObjectName(shared_class::DurakNextMoveError{
                    "Do not move. Game thinks you do not have to move. Please wait for other players to move."}));
        }
    } else {
        user.sendMsgToUser(objectToStringWithObjectName(shared_class::DurakNextMoveError{
                "table not in valid state. Game lookup is only supported for 1 vs. 1 players and if every player only place one card at once."}));
    }
};

auto const nextMoveChill = [](GameDependencies &gameDependencies,
                              std::tuple<shared_class::DurakNextMove, User &> const &durakNextMoveUser) {
    nextMove(gameDependencies, durakNextMoveUser, Chill{});
};
auto const nextMoveAskDef = [](GameDependencies &gameDependencies,
                               std::tuple<shared_class::DurakNextMove, User &> const &durakNextMoveUser) {
    nextMove(gameDependencies, durakNextMoveUser, AskDef{});
};
auto const nextMoveAskAttackAndAssist = [](GameDependencies &gameDependencies,
                                           std::tuple<shared_class::DurakNextMove, User &> const &durakNextMoveUser) {
    nextMove(gameDependencies, durakNextMoveUser, AskAttackAndAssist{});
};

auto const startAskAttackAndAssist = [](GameDependencies &gameDependencies,
                                        boost::sml::back::process<pauseTimer, nextRoundTimer, resumeTimer, sendTimerEv> process_event) {
    if (auto defendingPlayer = gameDependencies.game.getDefendingPlayer()) {
    ...
    if (auto assistingPlayer = gameDependencies.game.getAssistingPlayer(); assistingPlayer &&
        ...
        if (auto user = ranges::find_if(gameDependencies.users, [&assistingPlayer](auto const &_user) {
            ...
            user->sendMsgToUser(objectToStringWithObjectName(
                    shared_class::DurakDefendWantsToTakeCardsFromTableDoYouWantToAddCards{}));
            defendsWantsToTakeCardsSendMovesToAttackOrAssist(gameDependencies.game, durak::PlayerRole::assistAttacker,
                                                             *user);
        }
    } else {
        gameDependencies.passAttackAndAssist.assist = true;
...
auto const doPass = [](GameDependencies &gameDependencies, auto const &eventAndUser,
                       boost::sml::back::process<pauseTimer, sendTimerEv> process_event) {
    ...
    if (gameDependencies.game.getAttackStarted()) {
        if (gameDependencies.game.countOfNotBeatenCardsOnTable() == 0) {
            if (playerRole == durak::PlayerRole::attack || playerRole == durak::PlayerRole::assistAttacker) {
                ...
                user.sendMsgToUser(objectToStringWithObjectName(
                        eventErrorType{"account role is not attack or assist: " + playerName}));
            }
        } else {
...
auto const setAttackPass = [](GameDependencies &gameDependencies,
                              std::tuple<shared_class::DurakAttackPass, User &> const &durakAttackPassAndUser,
                              boost::sml::back::process<pauseTimer, sendTimerEv> process_event) {
    doPass(gameDependencies, durakAttackPassAndUser, process_event);
};

auto const setAssistPass = [](GameDependencies &gameDependencies,
                              std::tuple<shared_class::DurakAssistPass, User &> const &durakAssistPassAndUser,
                              boost::sml::back::process<pauseTimer, sendTimerEv> process_event) {
    doPass(gameDependencies, durakAssistPassAndUser, process_event);
};

auto const handleDefendSuccess = [](GameDependencies &gameDependencies,
                                    std::tuple<shared_class::DurakAskDefendWantToTakeCardsAnswer, User &> const &defendAnswerEventAndUser) {
...
auto const handleDefendPass = [](GameDependencies &gameDependencies,
                                 std::tuple<shared_class::DurakDefendPass, User &> const &durakDefendPassAndUser,
                                 boost::sml::back::process<askAttackAndAssist> process_event) {
    ...
    if (gameDependencies.game.getRoleForName(user.accountName) == durak::PlayerRole::defend) {
        if (gameDependencies.game.getAttackStarted()) {
            ...
            user.sendMsgToUser(
                    objectToStringWithObjectName(shared_class::DurakDefendPassError{"attack is not started"}));
        }
    } else {
        user.sendMsgToUser(objectToStringWithObjectName(
                shared_class::DurakDefendPassError{"account role is not defined: " + user.accountName}));
    }
};

auto const resetPassStateMachineData = [](
        GameDependencies &gameDependencies) { gameDependencies.passAttackAndAssist = PassAttackAndAssist{}; };

auto const tryToAttackAndInformOtherPlayers = [](GameDependencies &gameDependencies,
                                                 shared_class::DurakAttack const &attackEv,
                                                 durak::PlayerRole playerRole,
                                                 boost::sml::back::process<resumeTimer, pauseTimer, sendTimerEv> process_event,
                                                 bool isChill, User &user) {
    if (gameDependencies.game.playerAssists(playerRole, attackEv.cards)) {
        ...
        if (isChill) {
            ...
            sendGameDataToAccountsInGame(gameDependencies.game, gameDependencies.users, gameDependencies.opponentCards);
            auto otherPlayerRole = (playerRole == durak::PlayerRole::attack) ? durak::PlayerRole::assistAttacker
                                                                             : durak::PlayerRole::attack;
            defendsWantsToTakeCardsSendMovesToAttackOrAssist(gameDependencies.game, playerRole, user);
            if (auto otherPlayer = (otherPlayerRole == durak::PlayerRole::attack)
...
auto const doAttack = [](std::tuple<shared_class::DurakAttack, User &> const &durakAttackAndUser,
                         GameDependencies &gameDependencies,
                         boost::sml::back::process<resumeTimer, pauseTimer, sendTimerEv> process_event, bool isChill) {
    ...
    if (not gameDependencies.game.getAttackStarted() && playerRole == durak::PlayerRole::attack) {
        if (gameDependencies.game.playerStartsAttack(attackEv.cards)) {
            ...
            user.sendMsgToUser(
                    objectToStringWithObjectName(shared_class::DurakAttackError{"not allowed to play cards"}));
        }
    } else {
...
auto const doAttackChill = [](GameDependencies &gameDependencies,
                              std::tuple<shared_class::DurakAttack, User &> const &durakAttackAndUser,
                              boost::sml::back::process<resumeTimer, pauseTimer, sendTimerEv> process_event) {
    doAttack(durakAttackAndUser, gameDependencies, process_event, true);
};
auto const doAttackAskAttackAndAssist = [](GameDependencies &gameDependencies,
                                           std::tuple<shared_class::DurakAttack, User &> const &attackEventAndUser,
                                           boost::sml::back::process<resumeTimer, pauseTimer, sendTimerEv> process_event) {
    doAttack(attackEventAndUser, gameDependencies, process_event, false);
};
auto const doDefend = [](GameDependencies &gameDependencies,
                         std::tuple<shared_class::DurakDefend, User &> const &defendAndUser,
                         boost::sml::back::process<resumeTimer, pauseTimer, sendTimerEv> process_event) {
    auto [defendEvent, user] = defendAndUser;
    auto playerRole = gameDependencies.game.getRoleForName(user.accountName);
    if (playerRole == durak::PlayerRole::defend) {
        if (gameDependencies.game.playerDefends(defendEvent.cardToBeat, defendEvent.card)) {
            ...
            user.sendMsgToUser(objectToStringWithObjectName(shared_class::DurakDefendError{"Error while defending " +
                                                                                           fmt::format(
                                                                                                   "CardToBeat: {},{} vs. Card: {},{}",
                                                                                                   defendEvent.cardToBeat.value,
                                                                                                   magic_enum::enum_name(
                                                                                                           defendEvent.cardToBeat.type),
                                                                                                   defendEvent.card.value,
                                                                                                   magic_enum::enum_name(
                                                                                                           defendEvent.card.type))}));
        }
    }
};

auto const blockOnlyDef = [](
        std::tuple<shared_class::DurakAskDefendWantToTakeCardsAnswer, User &> const &defendWantToTakeCardsEventAndUser) {
    auto [event, user] = defendWantToTakeCardsEventAndUser;
    user.sendMsgToUser(objectToStringWithObjectName(shared_class::DurakAllowedMoves{{}}));
};

auto const attackErrorUserHasWrongRole = [](std::tuple<shared_class::DurakAttack, User &> const &durakAttackAndUser,
                                            GameDependencies &gameDependencies) {
    auto [event, user] = durakAttackAndUser;
    user.sendMsgToUser(objectToStringWithObjectName(shared_class::DurakAttackError{
            "Wrong role error. To attack you need to have the role attack or assist. Your role is: " +
            std::string{magic_enum::enum_name(gameDependencies.game.getRoleForName(user.accountName))}}));
};

auto const needsToBeDefendingPlayerError = [](
        std::tuple<shared_class::DurakAskDefendWantToTakeCardsAnswer, User &> const &askDefendWantToTakeCardsAnswerEventAndUser,
        GameDependencies &gameDependencies) {
    auto [askDefendWantToTakeCardsAnswerEvent, user] = askDefendWantToTakeCardsAnswerEventAndUser;
    user.sendMsgToUser(objectToStringWithObjectName(shared_class::DurakAskDefendWantToTakeCardsAnswerError{
            "Wrong role error. To take or discard cards you need to have the role defend. Your role is: " +
            std::string{magic_enum::enum_name(gameDependencies.game.getRoleForName(user.accountName))}}));
};

auto const wantsToTakeCards = [](
        std::tuple<shared_class::DurakAskDefendWantToTakeCardsAnswer, User &> const &askDefendWantToTakeCardsAnswerEventAndUser) {
    auto [askDefendWantToTakeCardsAnswerEvent, user] = askDefendWantToTakeCardsAnswerEventAndUser;
    return askDefendWantToTakeCardsAnswerEvent.answer;
};

auto const sendStartGameToUser = [](GameDependencies &gameDependencies) {
    ranges::for_each(gameDependencies.users, [&gameDependencies](User &user) {
        user.sendMsgToUser(objectToStringWithObjectName(matchmaking_game::StartGameSuccess{gameDependencies.gameName}));
    });
};
